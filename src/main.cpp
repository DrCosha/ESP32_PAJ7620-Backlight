...
// константа определяющая промежуток времени в течение которого должно придти подтверждение команды выключения от сенсора
// (режим блокировки случайных выключений от датчика движения)
#define PREVENT_MIS_OFF_DELAY 800                   // в течение 0.8 сек нужна повторная команда OFF от датчика движения для выключения
...
enum UserCommand {
  ...
  UCMD_WAIT_CMD_OFF                                 // включение ожидания выключения (блокировка от случайного выключения жестом)
};
...
bool WaitCMD_OFF = false;                           // флаг режима ожидания выключения
...
unsigned long LastOFFCommand = 0;                   // время получения последней команды на выключение
    ...
    case GES_DOWN:
      {
        ...
        #endif
        if (DeviceON) {  // если устройство включено - в начале включаем режим ожидания выключения и далее выключаем его, если посвторная команда приходит вовремя
          HasChanges = true;
          // анализируем, в каком режиме мы получили эту команду
          if (WaitCMD_OFF) { // если мы уже ждем повторную команду
            curr_Command = UCMD_OFF;            // выключаем устройство
            WaitCMD_OFF = false;                // снимаем флаг
          } else {
            curr_Command = UCMD_WAIT_CMD_OFF;   // иначе даем команду на ожидание выключения устройства
            LastOFFCommand = millis();          // запоминаем момент последней команды на выключение
            WaitCMD_OFF = true;                 // взводим флаг ожидания команды выключения
          }
        }     
        break;
      }
...
void get_timer_event() {  // --- обрабатываем временные задержки и генерим события от таймера ---
  ...
  // проверка выключения режима ожидания повторной команды выключения
  if (WaitCMD_OFF and ((millis()-LastOFFCommand)>PREVENT_MIS_OFF_DELAY)) {      // если последняя команда на выключение превысила время ожидания повторной команды
    LastOFFCommand = 0;                                       // сбрасываем временной промежуток
    WaitCMD_OFF = false;                                      // выключаем режим ожидания повторной команды на выключение
  }
